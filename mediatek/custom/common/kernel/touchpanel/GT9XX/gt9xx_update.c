...
GTP_DEBUG("----------------timing 1--------------");
	...
	//gtp_reset_guitar(guitar_client, 20);			//timing add 20130228
...
unsigned int ic_fw_vid;
u8 gup_get_ic_vid_pid()
{
    s32 ret = -1;
    u8  retry = 0;
    u8  buf[16];
    ...
    //step3:get pid & vid
    ret = gup_get_ic_msg(i2c_client_point, GUP_REG_PID_VID, buf, 6);
    ...
    update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH + 4] + (buf[GTP_ADDR_LENGTH + 5] << 8);
	ic_fw_vid = update_msg.ic_fw_msg.vid;
