...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("A5142AF", 0x19)};


#define A5142AF_DRVNAME "A5142AF"
#define A5142AF_VCM_WRITE_ID           0x19
...
static int s4A5142AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    i4RetValue = i2c_master_recv(g_pstA5142AF_I2Cclient, pBuff , 2);

    if (i4RetValue < 0) 
    {
        A5142AFDB("[A5142AF] I2C read failed!! \n");
        return -1;
    }
...
static int s4A5142AF_WriteReg(u16 a_u2Data)
{
    ...
    //A5142AFDB("[A5142AF] g_sr %d, write %d \n", g_sr, a_u2Data);
    g_pstA5142AF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstA5142AF_I2Cclient, puSendCmd, 2);
	
    if (i4RetValue < 0) 
    {
        A5142AFDB("[A5142AF] I2C send failed!! \n");
        return -1;
    }
